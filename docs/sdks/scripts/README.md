...
* [ListScripts](#listscripts) - Get a list of Script objects
...
func main() {
    ...
    ctx := context.Background()
    res, err := s.Scripts.ListScripts(ctx)
    if err != nil {
        log.Fatal(err)
    }

    if res.ScriptLibEntries != nil {
        // handle response
    }
}
```
...
**[*operations.ListScriptsResponse](../../models/operations/listscriptsresponse.md), error**
...
func main() {
    ...
    ctx := context.Background()
    res, err := s.Scripts.Update(ctx, id, requestBody)
    if err != nil {
        log.Fatal(err)
    }
